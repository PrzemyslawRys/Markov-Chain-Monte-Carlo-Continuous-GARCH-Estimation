...
data.frame(RVW = getRealizedVolatilitySeries(StoneColdClassicMins$R, 405 * 5, 1 / (252 * 405)),
           RVQ = getRealizedVolatilitySeries(StoneColdClassicMins$R, 405 * 63, 1 / (252 * 405)),
           volMCMC = StoneColdClassicMins$volEstimate,
           vol = StoneColdClassicMins$vol) %>% 
...
```{r, fig.width=15, fig.height=5}
data.frame(RVW = getRealizedVolatilitySeries(ChilloutRideMins$R, 405 * 5, 1 / (252 * 405)),
           RVQ = getRealizedVolatilitySeries(ChilloutRideMins$R, 405 * 63, 1 / (252 * 405)),
           volMCMC = ChilloutRideMins$volEstimate,
           vol = ChilloutRideMins$vol) %>%
  filter(row_number() %% 60 == 0) %>%
  mutate(time = row_number()) %>%
...
```{r, fig.width=15, fig.height=5}
data.frame(RVW = getRealizedVolatilitySeries(RollerCoasterDevilMins$R, 405 * 5, 1 / (252 * 405)),
           RVQ = getRealizedVolatilitySeries(RollerCoasterDevilMins$R, 405 * 63, 1 / (252 * 405)),
           volMCMC = RollerCoasterDevilMins$volEstimate,
           vol = RollerCoasterDevilMins$vol) %>%
  filter(row_number() %% 60 == 0) %>%
  mutate(time = row_number()) %>%
...
```{r}
rollerCoasterDevilStats[, 2 * (1:5) - 1] %>%
  kable("latex",
        booktabs = T,
